...
let elapsedTime = 0;
const myDiv = document.getElementById('count'); // Get the div element by ID
...
function startTimer() {
    ...
    // Update elapsed time and display on the frontend every second
    setInterval(() => {
      elapsedTime++;
      // Update the timer element with the elapsed time
      // Check if elapsed time is greater than 120 seconds (2 minutes)
      if(value == 8){
...
function endGame() {
    ...
    if (confirm("Time is up, reload page to play again")) {
        window.location.reload();
